var app = new function() {
        ...
        this.mostrarVuelos = function() {
          ...
          if (this.vuelos.length > 0) {
            for (i = 0; i < this.vuelos.length; i++) {
              ...
              data += '<tr>';
              data += '<td>Habitacion : '+ (i+1) + ' Tipo: ' + this.vuelos[i].origen +  ', Fecha de disponibilidad: ' + this.vuelos[i].hora.toDateString() + " " + hora + ":" + minutos + '</td>';
        ...
        this.Reservar = function (item) {
          ...
          document.getElementById('datosvuelo').innerHTML = "Habitacion " + (item + 1) + ":<br>Tipo: " + this.vuelos[item].origen +  '<br>Fecha de disponibilidad: ' + this.vuelos[item].hora.toDateString() + " " + hora + ":" + minutos + '<br>PRECIO BASE: $' + this.vuelos[item].costobase + '<br>PRECIO NETO: $' + this.vuelos[item].costoneto + " " + impuesto;
          ...
          document.getElementById('reserva-edit').onsubmit = function() {
            ...
            if (isNaN(d) || d == 0) {
              ...
              if (flag) {
                window.alert("Error: usted ya tiene reservada una habitacion para esta fecha");
              }else{
        ...
        this.consultarReserva = function () {
          ...
          if (isNaN(d) || d == 0) {
            ...
            var data = '<br>HABITACIONES RESERVADAS DE ' + d;
            for (i = 0; i < this.vuelos.length; i++) {
            ...
            if (data == '<br>HABITACIONES RESERVADAS DE ' + d) {
              window.alert("No existen habitaciones asociadas a dicho documento");
            }else{
              document.getElementById('menu1').style.display = 'none';
              document.getElementById('menu2').style.display = 'none';
              document.getElementById('vuelos').style.display = 'block';
